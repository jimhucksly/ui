...
```html
<b-dialog @modal-in-focus="onModalInFocus" />
```
```js
...
если в системе несколько компонентов b-dialog, например, в каждом микросервисе свой, то их необходимо локализовать с помощью id
...
- - метод должен вернуть результирующий объект или вызвать $emit('set-result', result);
```js
save() {
  return {
    name: 'Lanit'
...
* метод isChanged()
 ...
 - - метод нужен для определения, были ли внесены изменения. если в вызываемом компоненте определен метод isChanged, то он будет вызван при закрытии модального окна.
 - - если изменения были (метод должен вернуть true), то будет показано диалоговое окно для подтверждения закрытия окна
 - - если изменений не было (метод должен вернуть false), то окно будет просто закрыто.
...
в окне типа CreateEdit можно заблокировать кнопку Ok
