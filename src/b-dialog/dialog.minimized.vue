    ...
    <template v-else>
      <v-menu>
        <template #activator="{ props }">
          <component :is="$ui.options.aliases['b-button']" v-bind="props" class="minimize-btn">
        ...
        <v-card class="d-flex flex-column overflow-hidden" style="max-height: 65vh">
          <v-list>
            <v-list-item v-for="item in items" :key="item.id" @click="onMaximize(item)">
              ...
              <template #append>
                <component :is="$ui.options.aliases['b-button']" icon text @click.stop="onMaximize(item)">
                  <svg-icon>expand</svg-icon>
                </component>
                <component :is="$ui.options.aliases['b-button']" icon text @click.stop="onClose(item)">
                  <svg-icon>close</svg-icon>
                </component>
              </template>
            </v-list-item>
          ...
          <div class="pa-3">
            <component :is="$ui.options.aliases['b-button']" color="grey" variant="outlined" @click="onCloseAll">
              <svg-icon>close</svg-icon>
