...
export default class ResponsiveButtonComponent extends mixins(ViewportMixin) {
  ...
  get view(): string {
    if (this.desktop) {
      return 'b-button--desktop-view';
    }
    if (this.tablet) {
      return 'b-button--tablet-view';
    }
    if (this.mobile) {
      return 'b-button--mobile-view';
    }
  }

  get classes(): Array<string | Record<string, string>> {
    const result: Array<string | Record<string, string>> = ['b-button', this.view];
    if (this.text) {
      result.push('b-button-text');
    }
    if (this.mobile || this.tablet) {
      result.push('b-button-square');
    }
