<template>
  <v-breadcrumbs
    class="b-breadcrumbs"
    :class="[`b-breadcrumbs--${mySize}`, `b-breadcrumbs--theme-${theme}`]"
    :items="visibleItems"
  >
    <template #divider>
      ...
      <template v-else>
        <v-menu v-if="item.hidden">
          <template #activator="{ props }">
            <div
              v-bind="props"
              class="b-breadcrumbs-menu"
              :style="{ width: `${iconSize}px`, height: `${iconSize}px`, 'border-radius': `${iconSize}px` }"
            >
              <i>...</i>
            </div>
          </template>
          <v-list class="b-breadcrumbs-list" :class="[`v-list--${mySize}`, `b-breadcrumbs-list--${mySize}`]">
...
<script src="./b-breadcrumbs.ts"></script>
