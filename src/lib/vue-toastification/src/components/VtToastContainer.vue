...
export default defineComponent({
  ...
  computed: {
    toastArray(): Array<ToastOptionsAndRequiredContent> {
      return Object.values(this.toasts) as Array<ToastOptionsAndRequiredContent>
    },
    filteredToasts(): Array<ToastOptionsAndRequiredContent> {
      return this.defaults.filterToasts(this.toastArray) as Array<ToastOptionsAndRequiredContent>
    },
  },

  beforeMount() {
    eventBus.$on(EVENTS.ADD, this.addToast)
    eventBus.$on(EVENTS.CLEAR, this.clearToasts)
    eventBus.$on(EVENTS.DISMISS, this.dismissToast)
    eventBus.$on(EVENTS.UPDATE, this.updateToast)
    eventBus.$on(EVENTS.UPDATE_DEFAULTS, this.updateDefaults)
    this.defaults = this.$props as unknown as PluginOptionsType
  },

  beforeUnmount() {
    eventBus.$off(EVENTS.ADD, this.addToast)
    eventBus.$off(EVENTS.CLEAR, this.clearToasts)
    eventBus.$off(EVENTS.DISMISS, this.dismissToast)
    eventBus.$off(EVENTS.UPDATE, this.updateToast)
    eventBus.$off(EVENTS.UPDATE_DEFAULTS, this.updateDefault)
  },
  ...
  methods: {
    ...
    getPositionToasts(position: POSITION) {
      const toasts = this.filteredToasts
        .filter((toast: ToastOptionsAndRequiredContent) => toast.position === position)
